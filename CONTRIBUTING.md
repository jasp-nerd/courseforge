...
Thanks for wanting to make Canvas course creation better for everyone! Contributions of all kinds are welcome: bug reports, docs, tests, new tools, new question types, skill improvements.
...
| Package | What it is |
|---|---|
| `packages/shared` | `CourseSpec` Zod schemas — the common course format every pillar consumes |
...
We build this project *with* AI agents and welcome AI-assisted PRs, with one hard rule: **you must understand and stand behind every line you submit**. Please:

- Keep generations small and reviewed — no bulk-generated modules.
- Never commit secrets; Canvas tokens belong in `.env` (gitignored).
- Security-sensitive areas (ZIP handling, XML parsing, HTTP auth) get extra scrutiny — see the hardening rules in `CLAUDE.md`.
